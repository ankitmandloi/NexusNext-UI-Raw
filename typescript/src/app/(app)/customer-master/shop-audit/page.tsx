...
type ShopAuditForm = {
  ...
  auditTypeId: number | string
  auditRemark: string
  auditImageId: number | string
  visitDate: string
}

export default function ShopAuditPage() {
  const [audits, setAudits] = useState<ShopAudit[]>(initialShopAudits)
  const [activeModal, setActiveModal] = useState<ModalState>(null)
  const [selectedAudit, setSelectedAudit] = useState<ShopAudit | null>(null)
  const [currentPage, setCurrentPage] = useState(1)

  // Add form state
  const [form, setForm] = useState<ShopAuditForm>({
    customerId: '',
    employeeId: '',
    auditTypeId: '',
    auditRemark: '',
  ...
  // Edit form states
  const [editSelectedCustomer, setEditSelectedCustomer] = useState<{ id: number; name: string } | null>(null)
  const [editSelectedEmployee, setEditSelectedEmployee] = useState<{ id: number; name: string } | null>(null)
  const [editAuditImageFile, setEditAuditImageFile] = useState<File | null>(null)
  ...
  const openModal = (type: ModalState, payload?: ShopAudit) => {
    setActiveModal(type)
