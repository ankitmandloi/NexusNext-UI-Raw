...
type EventPlannerForm = {
  eventTypeId: string
  eventVenue: string
  eventRemark: string
  expectedInvitees: string
  budgetPerInvitee: string
  statusId: string
  companyRatio: string
  employeeId: string
  eventDate: string
...
export default function EventPlannerPage() {
  const [events, setEvents] = useState<EventPlanner[]>(initialEventPlanners)
  const [activeModal, setActiveModal] = useState<ModalState>(null)
  const [selectedEvent, setSelectedEvent] = useState<EventPlanner | null>(null)
  const [currentPage, setCurrentPage] = useState(1)

  // Add form state
  const [form, setForm] = useState<EventPlannerForm>({
    eventTypeId: '',
    eventVenue: '',
    eventRemark: '',
    expectedInvitees: '',
  ...
  const [budgetaryFile, setBudgetaryFile] = useState<File | null>(null)
  ...
  const openModal = (type: ModalState, payload?: EventPlanner) => {
    setActiveModal(type)
  ...
  const handleSaveEdit = (editForm: { eventRemark: string }) => {
          ...
          <div className="max-h-[60vh] overflow-y-auto pr-2">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                ...
                <Input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx" onChange={(e) => setBudgetaryFile(e.target.files?.[0] || null)} />
                {budgetaryFile && (
              ...
              {/* Event Venue */}
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Event Venue</label>
                <Input
                  type="text"
            ...
            {/* Event Remark (full width below) */}
            <div className="mt-6">
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Event Remark</label>
              <textarea
